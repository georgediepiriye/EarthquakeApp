...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            JSONObject rootJsonResponse = new JSONObject(SAMPLE_JSON_RESPONSE);
            JSONArray featuresJsonArray = rootJsonResponse.getJSONArray("features");
            for (int i = 0; i < featuresJsonArray.length(); i++) {
                JSONObject currentEarthquake = featuresJsonArray.getJSONObject(i);
                JSONObject propertyObject = currentEarthquake.getJSONObject("properties");

                double magnitude = propertyObject.getDouble("mag");
                String place = propertyObject.getString("place");
                long timeInMilliseconds = propertyObject.getLong("time");
